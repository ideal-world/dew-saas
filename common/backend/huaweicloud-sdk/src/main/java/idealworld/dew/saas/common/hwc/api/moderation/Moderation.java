...
public class Moderation extends BasicProcessor<Moderation> {
     ...
     */
    public TextModerationResult text(String text, TextCategory... categories) {
        String cate = Stream.of(categories).map(c -> "\"" + c.getKey() + "\"").collect(Collectors.joining(","));
        JsonNode response = respFilter(http.post(url + "/v1.0/moderation/text",
                "{\n"
                        + "   \"categories\":[" + cate + "],\n"
                        + "   \"items\":[\n"
                        + "      {\n"
        ...
        return new TextModerationResult()
                .setSuggestion(Suggestion.fromKey(result.get("suggestion").asText()).get())
                .setHitDetail(
     ...
     */
    public ImageModerationResult image(String imageUrl, ImageCategory... categories) {
     ...
     *
     * @param imageUrl   审核图片
     * @param threshold  结果过滤门限，只有置信度不低于此门限的结果才会呈现在detail的列表中，取值范围 0-1，当未设置此值时各个检测场景会使用各自的默认值。
     *                   <p>
     ...
     * @link https://support.huaweicloud.com/api-moderation/moderation_03_0019.html
     */
    public ImageModerationResult image(String imageUrl, float threshold, ImageCategory... categories) {
        String strThreshold = threshold == -1 ? "" : threshold + "";
        String cate = Stream.of(categories).map(c -> "\"" + c.getKey() + "\"").collect(Collectors.joining(","));
        JsonNode response = respFilter(http.post(url + "/v1.0/moderation/image",
                "{\n"
                        + "   \"categories\":[" + cate + "],\n"
                        + "   \"url\":\"" + imageUrl + "\",\n"
                        + "   \"image\":\"\",\n"
     ...
     */
    public List<ImageModerationResult> image(List<String> imageUrls, ImageCategory... categories) {
     ...
     * @link https://support.huaweicloud.com/api-moderation/moderation_03_0036.html
     */
    public List<ImageModerationResult> image(List<String> imageUrls, float threshold, ImageCategory... categories) {
        String strThreshold = threshold == -1 ? "" : threshold + "";
        String cate = Stream.of(categories).map(c -> "\"" + c.getKey() + "\"").collect(Collectors.joining(","));
        JsonNode response = respFilter(http.post(url + "/v1.0/moderation/image/batch",
                "{\n"
                        + "   \"categories\":[" + cate + "],\n"
    ...
    private ImageModerationResult packageImageModerationResult(String imageUrl, JsonNode result) {
                ...
                .setHitDetail(
                        $.fun.stream(result.get("detail").fields())
                                ...
                                .collect(Collectors.toMap(
                                        ...
                                        cateDetail -> new ImageModerationResult.CategoryDetail()
                                                .setSuggestion(
                                                        result.has("category_suggestions")
                                                                ? Suggestion.fromKey(
                                                                result.get("category_suggestions").get(cateDetail.getKey()).asText())
                                                ...
                                                .setDetails(
                                                        $.fun.stream(cateDetail.getValue().iterator())
                                                                ...
                                                                .map(detail ->
                                                                        new ImageModerationResult.CategoryDetail.Detail()
                                                                                .setConfidence(detail.get("confidence").asDouble())
                                                                                .setLabel(detail.get("label").asText()))
                                                                .collect(Collectors.toList())
                                                )
                                        )
                                )
